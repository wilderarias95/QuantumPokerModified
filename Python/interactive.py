...
class InteractiveContainer:
    ...
    def connectAllowedGates(self, interactiveButtons, allowedButtons):
        ...
        for i in range(len(self.gates)):
            ...
            if self.gates[i] == 'Y' and 'Y' in allowedButtonsKeys:
                self.connectGate('Y', interactiveButtons.Y)
            if self.gates[i] == 'S' and 'S' in allowedButtonsKeys:
                self.connectGate('S', interactiveButtons.S)
    ...
    def disconnectAllGates(self):
        ...
        for i in range(len(self.gates)):
            ...
            if self.gates[i] == 'Y' and 'Y' in connectedGates:
                self.disconnectGate('Y')
            if self.gates[i] == 'S' and 'S' in connectedGates:
                self.disconnectGate('S')
    ...
    def _motion(self, event):
        ...
        if self.currentAxes is not None:
            ...
            elif self.currentAxes == self.buttonsDict['Y'].ax:
                if not self.buttonsDict['Y'].ignore(event):
                    self.currentAxes.set_facecolor(self.buttonsDict['Y'].color)
            elif self.currentAxes == self.buttonsDict['S'].ax:
                if not self.buttonsDict['S'].ignore(event):
                    self.currentAxes.set_facecolor(self.buttonsDict['S'].color)
        ...
        if event.inaxes is not None:
            ...
            elif event.inaxes == self.buttonsDict['Y'].ax:
                if not self.buttonsDict['Y'].ignore(event):
                    event.inaxes.set_facecolor(self.buttonsDict['Y'].hovercolor)
            elif event.inaxes == self.buttonsDict['S'].ax:
                if not self.buttonsDict['S'].ignore(event):
                    event.inaxes.set_facecolor(self.buttonsDict['S'].hovercolor)
...
def createBellWindow(fig):
    bellProbs_ax = fig.add_subplot(position=(0.78, 0.001, 0.10, 0.210), anchor = "NW")
...
def createButtonsInfig(fig):
    basis_ax = plt.axes([0.88, 0.29, 0.1, 0.075])
    end_ax = plt.axes([0.78, 0.29, 0.1, 0.075])
    bell2_ax = plt.axes([0.78, 0.215, 0.1, 0.075])
    bell3_ax = plt.axes([0.88, 0.215, 0.1, 0.075])
    ...
    y_ax_p = plt.axes([0.78, 0.455, 0.1, 0.075])
    s_ax_p = plt.axes([0.78, 0.38, 0.1, 0.075])
    ...
    y_button_p = Button(y_ax_p, 'Y:   ')
    s_button_p = Button(s_ax_p, 'S:   ')
    ...
    buttonsDict = {'H': h_button_p, 'X': x_button_p, 'CX': cx_button_p, 'ZH': zh_button_p,
                   'CH': ch_button_p, 'Y': y_button_p, 'S': s_button_p, "Basis": basis_button, "End": end_button, "Bell2": bell2_button,
                   "Bell3": bell3_button}
    ...
    ch_patch = plt.Rectangle((0.88, 0.53), lw=0.75,
                            width=0.1, height=0.075, edgecolor='black', linewidth=0.75, facecolor='White',
                            transform=fig.transFigure)
    y_patch = plt.Rectangle((0.88, 0.455), lw=0.75,
                            width=0.1, height=0.075, edgecolor='black', linewidth=0.75, facecolor='White',
                            transform=fig.transFigure)
    s_patch = plt.Rectangle((0.88, 0.38), lw=0.75,
                            width=0.1, height=0.075, edgecolor='black', linewidth=0.75, facecolor='White',
                            transform=fig.transFigure)
    ...
    fig.patches.extend([y_patch])
    fig.patches.extend([s_patch])
    ...
    ch_text = fig.text(0.88+0.05, 0.53+0.07/2, 'CH: ',
                      horizontalalignment='center', verticalalignment='center', fontsize=12)
    y_text = fig.text(0.88+0.05, 0.455+0.07/2, 'Y: ',
                      horizontalalignment='center', verticalalignment='center', fontsize=12)
    s_text = fig.text(0.88+0.05, 0.38+0.07/2, 'S: ',
                      horizontalalignment='center', verticalalignment='center', fontsize=12)

    patchDict = {'H_patch': h_patch, 'X_patch': x_patch, 'CX_patch': cx_patch, 'ZH_patch': zh_patch,
                   'CH_patch': ch_patch, 'Y_patch': y_patch, 'S_patch': s_patch, 'H_text': h_text, 'X_text': x_text, 'CX_text': cx_text,                          'ZH_text': zh_text, 'CH_text': ch_text, 'Y_text': y_text, 'S_text': s_text}
