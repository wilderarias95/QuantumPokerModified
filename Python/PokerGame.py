...
class PokerGame:
    ...
    def endGame(self, allFolded=False):
        ...
        for i in range(self.nPlayers):
            ...
            job = execute(board.qc, backend=backend, shots=1024, optimization_level=3)
            job_monitor(job)
            counts = job.result().get_counts(board.qc)
            print(counts)
            estados = list(counts.keys())
            maximo = counts[estados[0]]
            key = estados[0]
            for j in range(len(estados)-1):
                if(maximo < counts[estados[j+1]]):
                    key = estados[j+1]
                    maximo = counts[estados[j+1]]
            for bitStr in key:
                if bitStr == "1":
                    scores[i] += 1
        print("\n---- Final scores----")
